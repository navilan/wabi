...
let
 stringTakeLast =
  ...
  buildApp = spec:
    let formats = [ "dmg" "pkg" "zip" ]; in
    with spec; pkgs.stdenv.mkDerivation rec {
      inherit name version;
      fileType = let extName = stringTakeLast url 3; in
        if builtins.elem extName formats then extName
        else abort "Unsupported compression file types: ${extName}";
      src = pkgs.fetchurl {
        url = builtins.replaceStrings [ "{0}" "{1}" "{2}" ] [ name version location ] url;
        sha256 = sha256;
...
in
{
  ...
  environment = {
    ...
    systemPackages = map buildApp fetched;
    variables = {
      EDITOR = "nvim";
      VISUAL = "nvim";
    };
